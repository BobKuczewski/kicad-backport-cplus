...
### 現在の C++ 実装の同期（2026-06-07）

現在の実装は KiCad 4/5 のレガシー境界を明示的に処理します。レガシー `.sch` は V6+ `.kicad_sch` に、レガシー `.lib` / `.dcm` は V6+ `.kicad_sym` に変換できます。V6+ `.kicad_sch` / `.kicad_sym` / `.kicad_pro` もレガシー `.sch` / `.lib` + `.dcm` / `.pro` に書き戻せます。V4 と V5 のレガシーファイル間では、現在は主に `.sch` と `.lib` のヘッダーを書き換え、元のレコードを保持します。

レガシー回路図変換は page metadata、components、fields、AR paths、wires、buses、bus entries、sheets、sheet pins、labels/text、junctions、no-connects を解析します。`L` または `F0` が `C?` のようなプレースホルダーのままの場合は、注釈済みの `AR Ref=` を優先します。隠し `#U...` 電源参照は可能な限り `#PWR` / `#FLG` に正規化します。レガシーシンボルライブラリ変換では、ライブラリシンボルの既定 `Reference` を接頭辞に正規化するため、`C?`、`R12`、`TP?` は `C`、`R`、`TP` として書かれます。一方、回路図上の配置済みインスタンスは `C12` のような具体的な参照と footprint property を保持します。

現在の `10.99` target alias は board/footprint 出力だけを `20260603` に進めます。symbol library は `20251024`、schematic は `20260306` のままです。これらの symbol/schematic の開発版ターゲットは、明示的な変換ターゲットになった時点で有効化します。

`CONVERTER::normalizeFile()` の主な流れは、legacy または S-expression 文書の検出、target alias または生の数値バージョンの解決、legacy から V4/V5 へのヘッダー書き換え、legacy から V6+ への S-expression 書き出し、modern から V4/V5 への legacy 書き出しと symbol library の `.dcm` sidecar 生成、modern S-expression 間でのコピー / upgrade normalization / downgrade rules の選択、最後に対象 `version` atom を書き込む、という順序です。

プロジェクト変換では生成物、history/backup、Gerber、BOM、一時ファイルを除外します。V4/V5 ターゲットでは modern `.kicad_prl` をコピーしません。V6/V7/V8 ターゲットでは numeric visible-items の `.kicad_prl` を生成します。V6 プロジェクトターゲットでは library-table のトップレベル `version` を削除し、ローカル `.pretty` footprint alias を補い、必要に応じて project-local `sym-lib-table` を作成し、生成した project-local symbol definitions を schematic cache に埋め込めます。
## 現在の開発バージョンのマトリックス
