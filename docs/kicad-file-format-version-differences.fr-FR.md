# Différences entre les versions du format de fichier KiCad

Ce texte est synchronisé structurellement avec la référence anglaise ; les termes techniques KiCad, tokens et noms de changements upstream restent volontairement inchangés.

Ce document suit les différences de versions du format de fichier KiCad utilisées par le
convertisseur de backport. Sa structure permet d'ajouter de nouvelles versions stables ou de
développement sans renommer le fichier.
...
- Tags GitLab officiels de KiCad et fichiers source.
- Checkout KiCad local dans `E:/WORKS/MY/kicadProject/kicad`.
- Local refs and tags: `origin/4.0`, `4.0.0`, `origin/5.0`, `origin/5.1`,
  `5.0.0`, `5.1.0`, `6.0.0`, `7.0.0`, `8.0.0`, `9.0.0`, `10.0.0`,
  and `origin/10.0`.
- `master` KiCad local, utilisé uniquement pour identifier les limites de branche de développement
  après 10.0.
- Implémentation `kicad-backport-cplus`, en particulier :
...
Les numéros de version proviennent des macros actives du code source KiCad :
...
- Les versions S-expression de circuit imprimé couvrent aussi les fichiers footprint `.kicad_mod`.
...
- Les schémas et bibliothèques de symboles KiCad 4/5 sont des fichiers legacy `.sch`, `.lib` et
  `.dcm`, pas des fichiers `.kicad_sch` ou `.kicad_sym`.

## Matrice principale des familles de fichiers
...
## Limite legacy KiCad 4/5

KiCad 4 et 5 ne sont pas simplement d'anciennes versions S-expression pour les données de schéma. Ils
utilisent une autre famille de fichiers pour les schémas et les bibliothèques de symboles :
...
Points de version PCB/footprint de KiCad 5 avant la ligne de développement KiCad 6 :
...
## Matrice de la version de développement actuelle

La branche KiCad `master` examinée est déjà passée au développement 11.0.
Ces constats sont des éléments de développement postérieurs à 10.0 et ne doivent pas être présentés
comme une prise en charge du format stable KiCad 10.0 :

| Type de fichier | Version de développement actuelle | Notes |
| --- | ---: | --- |
...
Étapes de développement postérieures à 10.0 trouvées jusqu'ici :
...
ajoute les étapes de format plus récentes suivantes :
...
## Synthèse des cibles de backport depuis les fichiers de développement actuels
...
## Couverture de l'implémentation C++ de backport

La CLI `kicad-backport-cplus` implémente des réécritures S-expression pilotées par version.
Elle résout les alias de release par type de document, applique les règles de downgrade, puis écrit
le champ `version` cible. Elle accepte aussi une version numérique brute du format de fichier
pour tester un cutoff précis du parseur.
...
Le convertisseur ne met pas les anciens fichiers à niveau. Si le fichier source possède une version
numérique inférieure à la version cible demandée, il copie le fichier sans changement
et conserve la version source dans le rapport.

### Détection des documents et gestion des projets

L'implémentation C++ détecte le type de document KiCad principalement à partir du head racine
de la S-expression :
...
Si le head racine est absent ou inconnu, la détection revient à l'extension de fichier :
...
### Règles des bibliothèques de symboles

Les gates génériques du parseur suppriment ces nœuds introduits lorsque le format cible
est antérieur à leur version d'introduction :

| Introduit | Heads supprimés | Raison |
| ---: | --- | --- |
...
### Règles de circuit imprimé et de footprint
...
| Introduit | Gestion de downgrade manquante | Notes |
| ---: | --- | --- |
...
### Feuille de dessin et règles de conception

La gestion des feuilles de dessin possède actuellement un gate de parseur implémenté :

| Seuil cible | Réécriture |
| ---: | --- |
| `< 20220228` | Remove worksheet `font` blocks |

Les règles de conception sont détectées et disposent d'alias de version cible, mais aucune réécriture de downgrade
n'est actuellement implémentée, car la macro de version du format de fichier reste
à `20200610` dans les versions de KiCad suivies.

### Sémantique des avertissements et des rapports

Chaque suppression implémentée ou réécriture de compatibilité qui modifie l'arbre ajoute un
avertissement. Les feature gates génériques indiquent le nombre de nœuds supprimés et la
version d'introduction. Les rapports incluent le chemin, le type détecté, la version source,
la version cible, le flag de modification et les avertissements.

## Exigences du convertisseur

### Chemin de lecture

- Préserver la `version` du fichier source ; ne pas l'interpréter uniquement comme format KiCad actuel.
- Prendre en charge les alias de compatibilité pour les anciens fichiers :
  - `page` to `paper`
  - Legacy overbar `~...~` to `~{...}`
  - Old `start/end` text box format to new `at/size`
  - Old `id` to `uuid`
  - Old boolean / presence-token formats to explicit booleans
- Détecter les formats futurs et retourner une erreur claire ou une stratégie de downgrade définie.
...
- Les downgrades avec perte doivent produire des avertissements ou des métadonnées sidecar au lieu de supprimer
  silencieusement.
...
Lors de l'ajout de futures différences de version :
