...
| `.kicad_pro` | Il JSON progetto moderno non viene riscritto per major version target. Per KiCad 6+ è normalmente copiato raw; per KiCad 5/4 viene generato un `.pro` minimo. |
...
| `.kicad_dru` | Il codice può rilevarlo e usa l’anchor fisso `20200610`, ma non è nella matrice principale dei formati visibili all’utente. |
...
| Stessa versione | Se source e target S-expression hanno la stessa versione, il file è copiato o lasciato invariato. | Evita riscritture inutili. |
...
| --- | --- | --- |
| `.kicad_pro` | Copia JSON raw per KiCad 6+, `.pro` minimo per KiCad 5/4. | Nessuna riscrittura JSON per major version target. |
| legacy `.pro` | Genera `.kicad_pro` JSON minimo per KiCad 6+. | Conserva solo setting legacy e nomi libreria riconosciuti. |
| `.kicad_sch` | Writer legacy `.sch` per KiCad 5/4; regole S-expression per KiCad 6+. | Proprietà, istanze e oggetti moderni complessi sono con perdita in legacy. |
...
| `.kicad_sym` | Scrive `.lib` e `.dcm` per KiCad 5/4. | Proprietà simbolo, grafica e simboli annidati moderni sono approssimati. |
