...
### 当前 C++ 实现同步（2026-06-07）

当前实现已经显式处理 KiCad 4/5 legacy 边界：legacy `.sch` 可以转换到 V6+ `.kicad_sch`，legacy `.lib` / `.dcm` 可以转换到 V6+ `.kicad_sym`，V6+ `.kicad_sch` / `.kicad_sym` / `.kicad_pro` 也可以写回 legacy `.sch` / `.lib` + `.dcm` / `.pro`。V4 与 V5 legacy 文件之间目前主要重写 `.sch` 和 `.lib` 文件头并保留原始记录。
...
项目转换会过滤生成物、history/backup、Gerber、BOM 和临时文件；V4/V5 目标不会复制现代 `.kicad_prl`；V6/V7/V8 目标会生成 numeric visible-items 的 `.kicad_prl`。V6 项目目标还会移除 library-table 顶层 `version`、补本地 `.pretty` footprint alias、必要时创建 project-local `sym-lib-table`，并可把生成的 project-local symbol definitions 嵌入 schematic cache。
## 当前开发版本矩阵
