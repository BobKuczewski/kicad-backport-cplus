...
| `.kicad_pro` | 现代工程 JSON 不按目标 KiCad 主版本重写；KiCad 6+ 目标通常原文复制，KiCad 5/4 目标才写最小 `.pro`。 |
| `.kicad_wks` | 识别并可改写 version；当前只有 worksheet `font` 降级规则，没有 KiCad 4/5 legacy worksheet writer。 |
| `.kicad_dru` | 代码可识别并有固定版本锚点，但本文不把它放入用户可见主格式矩阵。 |
| 转换粒度 | 同族 S-expression 转换是 AST 级规则重写；legacy 跨族转换是专用 reader/writer；工程目录转换还会补库表、嵌入符号和层级实例。 |
...
| 工程目录转换 | 复制可编辑工程输入文件，跳过备份/输出/制造目录；对文档文件按目标版本改扩展名并逐个转换；对库表和本地运行文件做少量项目级修正。 |

`.kicad_dru` 在代码中可以被识别为 `design-rules`，并有固定版本锚点 `20200610`；但它不作为本文的用户可见主格式矩阵描述对象。
...
| 快速复制 | S-expression 源版本与目标版本相同，或 `.kicad_pro` 目标仍为 KiCad 6+ 时，直接复制/原文输出。 | 避免无意义格式化和不必要规则扰动。 | 相同版本转换不会“清洗”文件；工程 JSON 不随目标 KiCad 6/7/8/9/10 主版本重写。 |
...
| 工程补全层 | 工程目录、库表、局部设置、层级实例、嵌入符号 | 文件逐个转换后，再做能让工程打开的项目级修正。 | `copyProjectTree()`、`ensureProjectLocalSymbolLibraryTable()`、`embedProjectLocalSchematicSymbols()`、`rebuildKiCad6ProjectHierarchyInstances()` |
...
| `.kicad_pro` | `6.0+` | 原文复制。 | `kicad-project-json`。 | 不按目标主版本改写 JSON 内容。 |
| `.kicad_pro` | `4.0`/`5.0`/`5.1` | 转为 `.pro`。 | `legacy-pro`。 | 只恢复 `legacy.project_settings` 和 legacy 符号库名；否则使用默认值或工程局部库名。 |
| `.kicad_sch` | `4.0`/`5.0`/`5.1` | 转为 `.sch`。 | `legacy-sch-v2` 或 `legacy-sch-v4`。 | 现代对象、属性、实例和图元能力被压缩到 legacy 可表达子集。 |
| `.kicad_sym` | `4.0`/`5.0`/`5.1` | 转为 `.lib`，同时写 `.dcm` sidecar。 | `legacy-lib-2.3` 或 `legacy-lib-2.4`。 | 符号属性、复杂图元、层级符号关系会按 legacy 记录近似。 |
...
| KiCad 6+ -> legacy `.sch` | 从现代节点写 `EESchema Schematic File Version 2/4`、`LIBS:`、`$Descr`、元件、字段、wire/bus、entry、label/text、sheet、sheet pin、junction、no-connect。 | 现代结构压缩到 legacy 记录，不能表达的属性和对象不完整保留。 |
...
| 降级到 `<20260306` | 移除 `embedded_fonts`；移除 sheet 的 `exclude_from_sim`、`in_bom`、`on_board`、`dnp`；移除顶层 `group`。 | 回退 KiCad 10 原理图能力。 |
...
| 降级到 `<=20230121` | 移除 `exclude_from_sim`；规范化 `uuid` atom 引号。 | 回退 KiCad 7 及更早解析器差异。 |
...
| 降级到 `<=20211123` | 移除 pin 的 `hide` 和 `alternate`；生成 KiCad 6 symbol instance table；标准属性补 id；sheet property 名称/id 规范化；移除 `instances`。 | 针对 KiCad 6 解析器的兼容修复。 |
...
| KiCad 6+ -> legacy `.lib/.dcm` | 写 `EESchema-LIBRARY Version 2.3/2.4`、`DEF`、`ALIAS`、标准/自定义 `F` 字段、`DRAW`、`X` pin；另写 `.dcm` 中的 `D/K/F`。 | 现代属性、嵌套符号和图元被压缩到 legacy 记录。 |
...
| 降级到 `<=20211014` | 移除 pin 的 `hide`，并补 KiCad 6 标准属性 id。 | 兼容 KiCad 6 初始符号库格式。 |
...
| 升级到 `>=20230410` | footprint `attr` 中的 `dnp` atom 展开为 boolean list。 | 适配较新 DNP 字段。 |
...
| KiCad 5 -> KiCad 4 | 原理图/符号库仍为 legacy；PCB/封装从 `20171130` 到 `4`。 | legacy `.sch/.lib` 只改头部；PCB/封装删除 KiCad 4 不支持字段并简化 custom pad。 | 差分对、keepout、自定义焊盘、3D offset 新写法等。 |
...
| KiCad 6 -> KiCad 5 | `.kicad_sch/.kicad_sym/.kicad_pro` 跨回 `.sch/.lib/.pro`；PCB/封装到 `20171130`。 | legacy writer；现代属性/实例/图元压缩；PCB header/layer/module/netcode/arc/rect 等回退。 | 现代属性、实例结构、复杂符号/原理图对象、现代 PCB 几何和层信息。 |
| KiCad 6 -> KiCad 7 | 文件族不变，版本提高到 KiCad 7 锚点。 | 升级规则只做旧字段规范化。 | 不生成源文件没有的新 DNP、仿真、图片、teardrop 等意图。 |
| KiCad 7 -> KiCad 6 | 文件族不变，版本降到 KiCad 6 锚点。 | 移除 KiCad 7-only 字段；补 KiCad 6 property id、sheet property、symbol instance 兼容结构。 | DNP、仿真字段、超链接、net tie、图片、stroke/dimension 等。 |
