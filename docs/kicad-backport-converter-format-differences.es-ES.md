...
| `.kicad_pro` | El JSON de proyecto moderno normalmente no se reescribe por versión mayor objetivo. Para KiCad 6/7/8 se vacían las URI worksheet embebidas `kicad-embed://...kicad_wks` en page layouts de proyecto; para KiCad 5/4 se genera un `.pro` legacy. |
...
| `.kicad_dru` | Los archivos de reglas de diseño se detectan y solo se copian si el objetivo soporta el mismo anchor fijo `.kicad_dru`. Los objetivos sin soporte `.kicad_dru` omiten el archivo con warning. |
...
| Misma versión | Si source y target S-expression tienen la misma versión, se copia o queda sin cambios. Las conversiones de proyecto aún pueden vaciar URI worksheet embebidas incompatibles para KiCad 6/7/8. | Evita churn de formato innecesario y corrige fallos conocidos de carga de proyecto. |
...
| --- | --- | --- |
| `.kicad_pro` | Copia JSON bruta para KiCad 6+; en conversión de proyecto a KiCad 6/7/8 se vacían las URI page-layout worksheet embebidas. `.pro` legacy mínimo para KiCad 5/4. | No reescribe JSON completo por versión mayor objetivo. |
| legacy `.pro` | Genera `.kicad_pro` JSON mínimo para KiCad 6+. | Conserva solo settings legacy y nombres de bibliotecas reconocidos. |
| `.kicad_sch` | Writer legacy `.sch` para KiCad 5/4; reglas S-expression para KiCad 6+. Las conversiones de proyecto añaden una referencia de cache library. | Propiedades, instancias y objetos modernos complejos son con pérdida en legacy; el texto multilínea se emite en una línea con `\n` escapado. |
...
| `.kicad_sym` | Escribe `.lib` y `.dcm` para KiCad 5/4; las conversiones de proyecto también copian la `.lib` generada a `<project>-cache.lib`. | Propiedades, gráficos y símbolos anidados modernos se aproximan; las referencias legacy `DEF` usan prefijos como `U`, `BT` o `#PWR`. |
...
| `.kicad_dru` | Se copia solo si el objetivo soporta el mismo anchor fijo de reglas de diseño. | No hay conversión de formato de reglas; los objetivos no soportados omiten el archivo con warning. |
...
## Correcciones de compatibilidad actuales
...
| Upgrade PCB/footprint | El `attr dnp` de footprint permanece como átomo `attr`; no se expande a `(dnp yes/no)`. |
