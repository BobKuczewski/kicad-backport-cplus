# Diferencias de versiones del formato de archivo de KiCad

Este texto está sincronizado estructuralmente con la referencia inglesa; los términos técnicos de KiCad, tokens y nombres de cambios upstream se mantienen sin traducir de forma deliberada.

Este documento registra las diferencias de versiones del formato de archivo de KiCad usadas por el
convertidor de backport. La estructura permite añadir versiones estables o de desarrollo nuevas
sin renombrar el archivo.
...
- Tags oficiales de GitLab de KiCad y archivos fuente.
...
- Local refs and tags: `origin/4.0`, `4.0.0`, `origin/5.0`, `origin/5.1`,
  ...
  and `origin/10.0`.
- `master` local de KiCad, usado solo para identificar los límites de la rama de desarrollo
  posterior a 10.0.
- Implementación de `kicad-backport-cplus`, especialmente:
  - `src/kicad_backport_versions.cpp`
  - `src/kicad_backport_rules.cpp`
  - `src/kicad_backport_rule_rewriters.cpp`
  - `src/kicad_backport.cpp`
- Archivos header de versión:
...
Los números de versión se toman de las macros activas del código fuente de KiCad:
...
- Las versiones S-expression de placa también cubren los archivos footprint `.kicad_mod`.
...
- Los esquemáticos y bibliotecas de símbolos de KiCad 4/5 son archivos legacy `.sch`, `.lib` y
  `.dcm`, no `.kicad_sch` ni `.kicad_sym`.

## Matriz principal de familias de archivo
...
## Límite legacy de KiCad 4/5

KiCad 4 y 5 no son simplemente versiones S-expression antiguas para datos de esquemático. Usan
una familia de archivos distinta para esquemáticos y bibliotecas de símbolos:
...
Puntos de versión PCB/footprint de KiCad 5 anteriores a la línea de desarrollo de KiCad 6:
...
## Matriz de la versión de desarrollo actual

La rama KiCad `master` revisada ya está en desarrollo 11.0.
Estos hallazgos son elementos de desarrollo posteriores a 10.0 y no deben etiquetarse como soporte
del formato estable de KiCad 10.0:
...
Pasos de versión de desarrollo posteriores a 10.0 encontrados hasta ahora:

| Versión | Tipo de archivo | Cambio |
| ---: | --- | --- |
...
## 10.0 a desarrollo actual

Comparada con los archivos objetivo de KiCad 10, la rama de desarrollo actual revisada
añade estos pasos de formato más recientes:
...
## Resumen de objetivos de backport desde archivos de desarrollo actuales
...
## Cobertura de la implementación C++ de backport

La CLI `kicad-backport-cplus` implementa reescrituras S-expression guiadas por versión.
Resuelve alias de release por tipo de documento, aplica reglas de downgrade y después escribe
el campo `version` objetivo. También acepta una versión numérica cruda del formato de archivo
para probar un cutoff concreto del parser.
...
El convertidor no actualiza archivos antiguos hacia versiones nuevas. Si el archivo fuente tiene una versión
numérica inferior a la versión objetivo solicitada, copia el archivo sin cambios
y conserva la versión fuente en el informe.

### Detección de documentos y manejo de proyectos

La implementación C++ detecta el tipo de documento KiCad principalmente desde el head raíz
de la S-expression:
...
Si falta el head raíz o es desconocido, se usa la extensión del archivo como fallback:
...
### Reglas de bibliotecas de símbolos

Los gates genéricos del parser eliminan estos nodos introducidos cuando el formato objetivo
es más antiguo que la versión de introducción:

| Introducido | Heads eliminados | Motivo |
| ---: | --- | --- |
...
### Reglas de esquemático

Gates genéricos del parser:
...
| Introducido | Manejo de downgrade faltante | Notas |
| ---: | --- | --- |
...
### Hoja de dibujo y reglas de diseño

El manejo de hojas de dibujo tiene actualmente un gate de parser implementado:

| Corte objetivo | Reescritura |
| ---: | --- |
| `< 20220228` | Remove worksheet `font` blocks |

Las reglas de diseño se detectan y tienen alias de versión objetivo, pero no hay reescrituras de downgrade
implementadas actualmente porque la macro de versión del formato de archivo permanece
en `20200610` en todas las versiones de KiCad rastreadas.

### Semántica de advertencias e informes

Cada eliminación implementada o reescritura de compatibilidad que cambia el árbol añade una
advertencia. Los feature gates genéricos informan el número de nodos eliminados y la
versión de introducción. Los informes incluyen ruta, tipo detectado, versión fuente,
versión objetivo, flag de cambios y advertencias.

## Requisitos del convertidor

### Ruta de lectura

- Conservar la `version` del archivo fuente; no interpretarlo solo como formato KiCad actual.
- Admitir alias de compatibilidad para archivos antiguos:
  - `page` to `paper`
  - Legacy overbar `~...~` to `~{...}`
  - Old `start/end` text box format to new `at/size`
  - Old `id` to `uuid`
  - Old boolean / presence-token formats to explicit booleans
- Detectar formatos futuros y devolver un error claro o una estrategia de downgrade definida.
...
- Los downgrades con pérdida deben producir advertencias o metadatos sidecar en lugar de eliminar
  en silencio.
...
Al añadir futuras diferencias de versión:
